...
/*typedef unsigned short bitchunk_t;#define CHAR_BIT 8  */
#define BITCHUNK_BITS   (sizeof(bitchunk_t) * CHAR_BIT) /* =8 */
/* 下面这个宏的意思就是计算 nr_bits 比特位 要用多少个bitchunk_t(unsigned short)来保存 */
#define BITMAP_CHUNKS(nr_bits) (((nr_bits)+BITCHUNK_BITS-1)/BITCHUNK_BITS)  
/* 取出bit位所在数组中的bitchunk_t */
