...
#define BuildMess(m_ptr, src, dst_ptr) \
	...
	switch (src) {							\
	case HARDWARE:							\ /*目标系统*/
		(m_ptr)->NOTIFY_ARG = priv(dst_ptr)->s_int_pending;	\ /*挂起中断副本*/
		priv(dst_ptr)->s_int_pending = 0;			\
		break;							\
	case SYSTEM:							\ /*目标系统*/
		(m_ptr)->NOTIFY_ARG = priv(dst_ptr)->s_sig_pending;	\ /*挂起信号副本*/
 ...
 */
  ...
  if (! (priv(caller_ptr)->s_trap_mask & (1 << function)) || 
          (iskerneln(src_dst) && function != SENDREC
           && function != RECEIVE)) { 
      kprintf("sys_call: trap %d not allowed, caller is %d, src_dst is %d\n", 
  ...
  if (! (isokprocn(src_dst) || src_dst == ANY || function == ECHO)) { 
      kprintf("sys_call: invalid src_dst, src_dst is %d, caller is %d\n", 
   ...
   * anywhere in data or stack or gap(是什么). It will have to be made more elaborate    
   * (详细的) for machines which don't have the gap mapped. 
   */
  if (function & CHECK_PTR) {	
      vlo = (vir_bytes) m_ptr >> CLICK_SHIFT; /* 消息开始的虚拟地址 */
      vhi = ((vir_bytes) m_ptr + MESS_SIZE - 1) >> CLICK_SHIFT;/* 消息结束的虚拟地址 */
 ...
 */
  ...
  /* (死锁检查)Check for deadlock by 'caller_ptr' and 'dst' sending to each other. */
  xp = dst_ptr;
  /* 这里为什么要用一个循环呢(因为你在等我，我在等他，他(要找到他)在等你,而我要给你发) */
  ...
  if ( (dst_ptr->p_rts_flags & (RECEIVING | SENDING)) == RECEIVING &&
	...
	if (caller_ptr->p_rts_flags == 0) dequeue(caller_ptr); /* 在其它值的时候已经是阻塞状态了 */ 
	caller_ptr->p_rts_flags |= SENDING;		       /* 加上阻塞的原因 */
 ...
 */
  ...
  if (!(caller_ptr->p_rts_flags & SENDING)) {
    ...
    /* Check if there are pending notifications, except for SENDREC. */
       /* (如果s_flags中SENDREC_BUSY没有置位)检测是否有挂起的通知 */
 ...
 * kernel is not reentrant(可重入的), which means to interrupts are disabled after 
 ...
 * insert it.  As a side-effect(副作用) the process' priority may be updated.  
 */
  /*注意,下面这个是一个静态变量  */
  static struct proc *prev_ptr = NIL_PROC;	/* previous without time */

  /* 
   * time_left,决定该进程是加在队列首还是尾：
   * 如果time_left=1 ,说明时间片段没有用完,那么该进程上次运行了足够的时间,所有应该把他加到队列尾
   * 如果time_left=0 ,说明上次该进程阻塞是由于它的运行时间不够了。那么这次它还应该接着运行，所有要
   * 把它加到队列首.
   */
  int time_left = (rp->p_ticks_left > 0);	/* quantum fully consumed(完全耗尽) */
  int penalty = 0;				/* penalty:处罚,change in priority */
   ...
   * 内核进程(时钟和系统任务)是不受影响的，但所有其他的进程优先级都可以降低
   * penalty 为正：优先级降低，反之增高
   */
