...
struct priv {
  ...
  reg_t *s_stack_guard;		/* stack guard(守卫者) word for kernel tasks */
};
...
#define PREEMPTIBLE	0x01	/* kernel tasks are not preemptible (抢战)*/
#define BILLABLE	0x04	/* some processes are not billable (可审计:能记录使用时钟数)*/
#define SYS_PROC	0x10	/* system processes are privileged(系统) */
