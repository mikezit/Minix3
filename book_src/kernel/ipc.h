...
#define CHECK_PTR       0x0B	/* 1 0 1 1 : validate message buffer(SEND,RECEIVE,SENDREC,ECHO,这几个要用到消息指针)*/
#define CHECK_DST       0x05	/* 0 1 0 1 : validate message destination(SEND,SENDREC,NOTIFY) */
#define CHECK_SRC       0x02	/* 0 0 1 0 : validate message source(RECEIVE,SENDREC) */
