...
PUBLIC int do_sigaction()
{
  ...
  svp = &mp->mp_sigact[m_in.sig_nr]; /* 得到该进程原来的sigaction */
  /* 把旧的sigaction存到sig_osa中去 */
  ...
  if (svec.sa_handler == SIG_IGN) {  /* ignore signal */
	sigaddset(&mp->mp_ignore, m_in.sig_nr); /* set to 1 */
	sigdelset(&mp->mp_sigpending, m_in.sig_nr); /* reset */
	sigdelset(&mp->mp_catch, m_in.sig_nr);	    /* reset */
	sigdelset(&mp->mp_sig2mess, m_in.sig_nr);   /* reset */
  } else if (svec.sa_handler == SIG_DFL) {  /* default signal handling */
	sigdelset(&mp->mp_ignore, m_in.sig_nr);
	sigdelset(&mp->mp_catch, m_in.sig_nr);
	sigdelset(&mp->mp_sig2mess, m_in.sig_nr);
  } else if (svec.sa_handler == SIG_MESS) { /* pass as message (MINIX) */
	if (! (mp->mp_flags & PRIV_PROC)) return(EPERM);
	sigdelset(&mp->mp_ignore, m_in.sig_nr);
	sigdelset(&mp->mp_catch, m_in.sig_nr);
	sigaddset(&mp->mp_sig2mess, m_in.sig_nr);
  } else {			/* 对于其他的信号扑作函数,catch置位 */
	sigdelset(&mp->mp_ignore, m_in.sig_nr);
 ...
 */
  ...
  slot = (int) (rmp - mproc);
  if ((rmp->mp_flags & (IN_USE | ZOMBIE)) != IN_USE) { /* 这里的意思是,这个进程状态是IN_USE,同时也有可能是ZOMBIE,这里保证这个进程的状态只是IN_USE*/
  ...
  if (sigismember(&rmp->mp_ignore, signo)) { /* 忽略这个信号 */
  	return;
  }
  if (sigismember(&rmp->mp_sigmask, signo)) { /* 阻塞这个信号 */
	/* Signal should be blocked. */
	sigaddset(&rmp->mp_sigpending, signo);
	return;
  }
  sigflags = rmp->mp_sigact[signo].sa_flags;
  if (sigismember(&rmp->mp_catch, signo)) { /* 捕捉这个信号 */
	if (rmp->mp_flags & SIGSUSPENDED)
		sm.sm_mask = rmp->mp_sigmask2;
	else
		sm.sm_mask = rmp->mp_sigmask;
	sm.sm_signo = signo;
	sm.sm_sighandler = (vir_bytes) rmp->mp_sigact[signo].sa_handler; /* 一个指针 */
	...
	rmp->mp_sigmask |= rmp->mp_sigact[signo].sa_mask; /* signals to be blocked during handler */

	if (sigflags & SA_NODEFER) /* don't block current process signal while catching it */
		sigdelset(&rmp->mp_sigmask, signo);
	else
		sigaddset(&rmp->mp_sigmask, signo);

	if (sigflags & SA_RESETHAND) { /* reset signal handler when signal caught */
		sigdelset(&rmp->mp_catch, signo);
		rmp->mp_sigact[signo].sa_handler = SIG_DFL;
	}

	if (OK == (s=sys_sigsend(slot, &sm))) { /* 开始交给内核处理 */
